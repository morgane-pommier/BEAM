...
if (utils::packageVersion("ggplot2") >= "2.0.0") {
  ...
  labeller(
    .rows = as_labeller(stringr::str_to_upper),
    .cols = as_labeller(stringr::str_to_upper)
  )
} else {
...
Facet_Effort <- ggplot(Effort_long, aes(x = ecoregion, y = proportion, fill = suitability)) +
  geom_bar(stat = "identity", position = "fill", width = .7) +
  labs(x = "", y = "Relative proportion of suitable vs. unsuitable effort (total DaS)",fill=NULL)  +coord_flip() + 
  scale_fill_manual(values = c("#f48c06", "#ffba08", "#dc2f02", "#80b918"),guide = guide_legend(reverse = TRUE))+
  theme(legend.position = "top")+
  theme(strip.background = element_rect(fill="white"),panel.background = element_rect(fill = "grey95"), axis.title=element_text(size=10), 
	axis.text = element_text(size=7),axis.text.x = element_text(size=6))+
  facet_grid(taxon~metierl4, labeller = capitalize_labeller  )+
  scale_y_continuous(breaks = c(0,0.5,1), labels = c(0,0.5,1))+ scale_x_discrete(labels=str_to_title)
...
Facet_bycatch <- ggplot(bycatch, aes(x = ecoregion, fill = n_ind, y= 1)) +
  ...
  labs(x = "", y = "", fill = "")+coord_flip()+theme(legend.position = "top", strip.background = element_rect(fill="white"),panel.background = element_rect(fill = "grey95"), axis.title=element_text(size=10), axis.text = element_text(size=7),axis.text.x = element_blank())+
  facet_grid(taxon~metierl4, labeller = capitalize_labeller)+ scale_x_discrete(labels=str_to_title)+
...
Facet_n_ind <- ggplot(N_ind_long, aes(x = ecoregion, y = proportion, fill = suitability)) +
  ...
  labs(x = "", y = "Relative proportion of suitable vs. unsuitable N_ind (total n_ind)",fill=NULL)  +coord_flip() + scale_fill_manual(values = c("#f48c06", "#ffba08", "#dc2f02", "#80b918"),
                                                                                                                                     guide = guide_legend(reverse = TRUE))+theme(legend.position = "top")+theme(strip.background = element_rect(fill="white"),panel.background = element_rect(fill = "grey95"), axis.title=element_text(size=10), axis.text = element_text(size=7),axis.text.x = element_text(size=6))+
																																	 facet_grid(taxon~metierl4, labeller = capitalize_labeller)+scale_y_continuous(breaks = c(0,0.5,1),labels = c(0,0.5,1))+ scale_x_discrete(labels=str_to_title)
...
Total_bycatch_estimated_BPUE_available <-ggplot(data[bpue_available==1], aes(x = ecoregion, fill = as.factor(message))) +
  geom_bar(position = "fill", col="grey80", width=.7) + 
  coord_flip() + 
  labs(y="Relative proportion of species for which a BPUE is available", x="", 
	fill="Total bycatch estimate available")+
	coord_flip() + 
	scale_fill_manual(values = c( "#80b918","#ffba08","#ef233c"), 
		labels = c("Yes", "No, Levels of at least one random effect not available", "No, Random effect on monitoring or sampling protocol"))+
	theme(legend.position = "top")+
	theme(strip.background = element_rect(fill="white"),panel.background = element_rect(fill = "grey95"), 
		axis.title=element_text(size=10), axis.text = element_text(size=7),axis.text.x = element_text(size=6))+
	facet_grid(taxon~metierl4,drop=FALSE, labeller = capitalize_labeller)+
	scale_y_continuous(breaks = c(0,0.5,1),labels = c(0,0.5,1))+ scale_x_discrete(labels=str_to_title)
...
data[message=="Fishing data available at all levels" & bycatch_estimated == "no", message := "Fishing data available at all levels, but no total bycatch estimated"]

Total_bycatch_estimated_BPUE_available_reason <-ggplot(data[bpue_available==1], aes(x = ecoregion, fill = as.factor(message))) +
  geom_bar(position = "fill", col="grey80", width=.7) + coord_flip() + 
  labs(y="Relative proportion of species for which a BPUE is available", x="", fill="Total bycatch estimate available")+coord_flip() + scale_fill_manual(values = c( "#80b918","#ffba08","#f48c06", "#ef233c"), labels = c("Yes", "No, Levels of at least one random effect not available", "No, Random effect on monitoring or sampling protocol", "No, fishing data available at all levels, but no total bycatch estimated"))+theme(legend.position = "top")+theme(strip.background = element_rect(fill="white"),panel.background = element_rect(fill = "grey95"), axis.title=element_text(size=10), axis.text = element_text(size=7),axis.text.x = element_text(size=6))+facet_grid(taxon~metierl4,drop=FALSE, labeller = capitalize_labeller)+scale_y_continuous(breaks = c(0,0.5,1),labels = c(0,0.5,1))+ scale_x_discrete(labels=str_to_title)+guides(fill=guide_legend(nrow=2,byrow=TRUE))

ggsave(Total_bycatch_estimated_BPUE_available_reason,file="Total_bycatch_estimated_BPUE_available_reason.png", width = 297, height = 210, units = "mm")
