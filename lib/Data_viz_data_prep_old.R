...
fishing_2024 <- fishing[year == 2024]
...
setDT(bycatch)[individualswithpingers < 0, individualswithpingers := NA] #minus values = NA
setDT(bycatch)[incidentswithpingers < 0, incidentswithpingers := NA] #minus values = NA
...
monitoring = monitoring[!is.na(ecoregion) & !is.na(country), .(daysatsea = sum(daysatseaob, na.rm = TRUE)),
...
total_bycatch[, bycatch_estimated := ifelse(is.na(mean), "no", "yes")]
